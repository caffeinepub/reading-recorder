...
export class Backend implements backendInterface {
    ...
    async getAllRecordingIds(): Promise<Array<RecordingId>> {
        if (this.processError) {
            try {
                const result = await this.actor.getAllRecordingIds();
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
    ...
    async getRecording(arg0: RecordingId): Promise<Recording> {
        if (this.processError) {
            try {
